...
namespace
{
  ...
  QRegularExpression park_id_re {"^[A-Za-z]{2}[-][0-9]{4}"};
...
QString Configuration::lotw_pwd () const {return m_->lotw_pwd_;}      //avt 4/2/25
QString Configuration::park_id () const {return m_->pota_id_.size() ? m_->pota_id_ : "PARK_ID";}      //avt 4/24/25
...
void Configuration::setSpecOpPota()
{ 
  m_->bSpecialOp_=true;
  m_->ui_->gbSpecialOpActivity->setChecked(m_->bSpecialOp_);
  m_->ui_->rbPota->setChecked(true);
  m_->SelectedActivity_ = static_cast<int> (SpecialOperatingActivity::POTA);
  m_->write_settings();
}

void Configuration::setSpecOpNone()
{ 
  ...
  , default_audio_output_device_selected_ {false}
{
  ...
  if (screenSize.height() <= 600) this->setFixedSize(QSize(650, screenSize.height() - 20));   //avt 3/26/25
  ...
  ui_->Park_ID->setValidator (new QRegularExpressionValidator {park_id_re, this});
  ...
  ui_->special_op_activity_button_group->setId (ui_->rbPota, static_cast<int> (SpecialOperatingActivity::POTA));    //avt 4/24/25
...
void Configuration::impl::initialize_models ()
{
  ...
  ui_->lotw_edit->setText (lotw_pwd_);      //avt 4/2/25
  ui_->Park_ID->setText (pota_id_);      //avt 4/24/25
...
void Configuration::impl::read_settings ()
{
  ...
  lotw_pwd_ = settings_->value ("Lotw_pwd", QString {}).toString ();    //avt 4/2/25
  pota_id_ = settings_->value ("Pota_id", QString {}).toString ();    //avt 4/24/25
  ...
  autoLog_ = settings_->value ("AutoLog", true).toBool ();      //avt 4/23/25
  insert_blank_ = settings_->value ("InsertBlank", true).toBool ();      //avt 4/23/25
  DXCC_ = settings_->value ("DXCCEntity", true).toBool ();      //avt 4/23/25
  ppfx_ = settings_->value ("PrincipalPrefix", false).toBool ();
  clear_DX_ = settings_->value ("ClearCallGrid", false).toBool ();
  miles_ = settings_->value ("Miles", true).toBool ();      //avt 4/23/25
  quick_call_ = settings_->value ("QuickCall", true).toBool ();      //avt 4/23/25
  ...
  if (SelectedActivity_ >= (static_cast<int> (SpecialOperatingActivity::INVALID)) or SelectedActivity_ <= 0) {    //downward compatibility
    SelectedActivity_ = 1;      //avt 4/23/25
    bSpecialOp_ = false;
  }
...
void Configuration::impl::write_settings ()
{
  ...
  settings_->setValue ("Lotw_pwd", lotw_pwd_);      //avt 4/2/25
  settings_->setValue ("Pota_id", pota_id_);      //avt 4/24/25
...
bool Configuration::impl::validate ()
{
  ...
  if (ui_->rbRTTY_Roundup-> isChecked () &&
    ...
    if (ui_->rbPota-> isChecked () &&
        ui_->Park_ID->text().size() &&
        !ui_->Park_ID->hasAcceptableInput ())
    {
      for (auto * parent = ui_->Park_ID->parentWidget (); parent; parent = parent->parentWidget ())
        {
          auto index = ui_->configuration_tabs->indexOf (parent);
          if (index != -1)
            {
      ...
      ui_->Park_ID->setFocus ();
      MessageBox::critical_message (this, tr ("Invalid Park ID")
                                    , tr ("You may leave 'Park ID' blank, or input a valid Park ID.\n\nExample: US-4403"));
      return false;
    }
...
void Configuration::impl::accept ()
{
  ...
  lotw_pwd_ = ui_->lotw_edit->text ();      //avt 4/2/25
  pota_id_ = ui_->Park_ID->text ().toUpper ();      //avt 4/24/25
